...
__global__ void bayer_reverse_kernel( uint8_t* input,int width,int height,int colorWidthStep)
{
  ...
  if((xIndex<width) && (yIndex<height)&& (yIndex >= 0) && (xIndex >= 0))
  {
    ...
    // input[color_tid + 2] = t;
    // int cnt=0;
    for(int abc=0;abc<BUSYNUM;abc++){
      if(yIndex % 2==0){
        if(xIndex  % 2==0){
          input[color_tid + 0]=input[color_tid + 1];
          // tmp.at<u_char>(yIndex,xIndex) = channels[1].at<u_char>(yIndex,xIndex);
        }
          
        else{
          input[color_tid + 0]=input[color_tid + 2];
          // tmp.at<u_char>(yIndex,xIndex) = channels[2].at<u_char>(yIndex,xIndex);
        }
          
      }else{
        if(xIndex  % 2==0){
          input[color_tid + 0]=input[color_tid + 0];
          // tmp.at<u_char>(yIndex,xIndex) = channels[0].at<u_char>(yIndex,xIndex);
        }
...
Mat bayer_reverse(Mat img){
  int width = img.size().width;
  int height = img.size().height;
  // cout<<"img type"<<img.type()<<"width"<<width<<"height"<<height<<endl;
  Mat tmp(cv::Size(width, height), CV_8U ); 
  vector<Mat> channels(3);
  split(img, channels);
  #pragma omp parallel for
  for(int i = 0;i<height;i++){
    for(int j = 0;j<width;j++){
      ...
      for(int abc=0;abc<BUSYNUM;abc++){
        if(i % 2==0){
          if(j % 2==0)
            tmp.at<u_char>(i,j) = channels[1].at<u_char>(i,j);
          else
            tmp.at<u_char>(i,j) = channels[2].at<u_char>(i,j);
        }else{
          if(j % 2==0)
            tmp.at<u_char>(i,j) = channels[0].at<u_char>(i,j);
          else
            tmp.at<u_char>(i,j) = channels[1].at<u_char>(i,j);
        }
        //tmp.atf(i,j)=(channels[0]).atf(i,j);
...
int main(int argc, char* argv[]) {
  ...
  double start,out_time,sum=0.0;
  // 讀取影像檔案
  Mat image,imageFloat;
  image = imread( argv[1] ,cv::IMREAD_COLOR);
  // cout<<"image type"<<image.type()<<endl;//16 CV_8UC3
  ...
  for(int i=0;i<TESTITERAION;i++){
    ...
    // start = CycleTimer::currentSeconds();
    // tmp =bayer_reverse2(small);
    // // tmp = bayer_reverse(small);
    // out_time = CycleTimer::currentSeconds() - start;
    // tmp.convertTo(tmpfloat, CV_32FC3, 1/255.0);
