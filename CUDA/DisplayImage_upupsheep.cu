...
__global__ void bayer_reverse(const PtrStepSzf img, PtrStepSzf dst, int height, int width, int colorWidthStep){
  ...
  if (i < width && j < height && && i >= 0 && j >= 0) {
    int color_tid = j * colorWidthStep + (3 * i);
    if(i % 2==0){
        if(j % 2==0)
          dst(j, i) = img(color_tid + 1 + j, i);
        else
          dst(j, i) = img(color_tid + 2 + j, i);
      }else{
        if(j % 2==0)
          dst(j, i) = img(color_tid + 0 + j, i);
        else
          dst(j, i) = img(color_tid + 1 + j, i);
      }
...
int main(int argc, char* argv[]) {
  ...
  GpuMat dst, src;
  src.upload(small);
  Size small_size = small.size();
  int height = small_size.height;
  int width = small_size.width;
  ...
  const dim3 grid_size((height + block_row - 1) / block_row, (width + block_row - 1) / block_row);
  ...
  for(int i=0;i<5;i++){
    ...
    bayer_reverse<<<grid_size, block_size>>>(src, dst, height, width, small.step);
    tmp = dst;
    out = DZ(tmp); 
