...
Mat bayer_reverse(Mat img){
  ...
  Mat tmp(cv::Size(width, height), CV_8UC1 ); 
  ...
  for(int i = 0;i<height;i++){
    for(int j = 0;j<width;j++){
      ...
      for(int abc=0;abc<BUSYNUM;abc++){
        if(i % 2==0){
          if(j % 2==0)
            tmp.at<char>(i,j) = channels[1].at<char>(i,j);
          else
            tmp.at<char>(i,j) = channels[2].at<char>(i,j);
        }else{
          if(j % 2==0)
            tmp.at<char>(i,j) = channels[0].at<char>(i,j);
          else
            tmp.at<char>(i,j) = channels[1].at<char>(i,j);
        }
